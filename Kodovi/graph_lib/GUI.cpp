...
void Button::set_color(int r, int g, int b) {
    pw->labelcolor(fl_rgb_color(r, g, b));
}

void RadioButton::attach(Window& win)
{
    pw = new Fl_Radio_Button(loc.x, loc.y, width, height, label.c_str());
    pw->callback(reinterpret_cast<Fl_Callback*>(do_it), &win); // pass the window
    own = &win;
}
